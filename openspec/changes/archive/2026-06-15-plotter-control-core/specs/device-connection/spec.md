...
The system SHALL connect to the plotter over the Web Serial API at 115200 baud, and SHALL allow the operator to select the serial port. It SHALL surface connection failures (port busy, permission denied, disconnected) as actionable errors.

#### Scenario: Successful connection

- **WHEN** the operator selects the plotter's serial port and connects
- **THEN** the port opens at 115200 baud and the app transitions to a connected state

#### Scenario: Port unavailable

- **WHEN** the selected port is already in use by another application
- **THEN** the app reports a clear "port busy / in use" error and remains disconnected
...
The system SHALL allow the operator to disconnect, releasing the serial port and stopping all background activity (status polling, streaming) cleanly.

#### Scenario: Clean disconnect

- **WHEN** the operator disconnects
- **THEN** status polling stops, any active stream is halted, and the serial port is released so other applications can use it
