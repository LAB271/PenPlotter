...
Tell the operator how long a plot will take — an estimated total before plotting and the
remaining time while it runs — derived from the generated program and the machine's motion
characteristics, shown cleanly on phone and laptop. This replaces the opaque G-code line
counter with a number the operator can act on.
...
The readout SHALL be derived from data shared across devices (the placed artwork and the
broadcast plot progress) so any connected device shows it, not only the device that
started the plot.
...
#### Scenario: Remaining time shown on a second device

- **WHEN** a plot started on one device is viewed on another (e.g. a phone watching a laptop-started plot)
- **THEN** that device shows the remaining-time estimate tracking progress, not a stuck zero
